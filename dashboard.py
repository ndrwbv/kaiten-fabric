...
def rules(cfg: dict) -> list[dict]:
    ...
    # Как он спрашивает — зависит от доски, и врать тут нельзя: на доске команды
    # колонки «Вопрос от агента» нет вовсе, там он ведёт себя как человек — блокирует
    # карточку и спрашивает комментарием.
    asking = []
    for profile in profiles:
        if factory.needs_blocker(profile, "question"):
            asking.append(f"На доске «{profile['key']}» делает это как человек: вешает "
                          f"на карточку блокер и пишет вопрос комментарием. Снял "
                          f"блокер — он вернётся и продолжит с твоим ответом.")
        else:
            asking.append(f"На доске «{profile['key']}» под это есть своя колонка: "
                          f"карточка уезжает в «Вопрос от агента» и ждёт. Двигать её "
                          f"руками не надо — вернётся в работу, когда последнее слово "
                          f"в карточке окажется твоим.")
    blocks.append({"title": "Чей сейчас ход", "lines": [
        "Не хватило данных — не гадает и не выбирает за продукт: спрашивает и ждёт.",
        *asking,
        "Свой блокер он не снимает никогда: снятие блокера человеком и есть "
        "«продолжай». Другого признака согласия нет — и он, и ты пишете в Kaiten "
        "под одним токеном, по автору реплики не различить.",
    ]})
